...
class NewReviewViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func onSave(_ sender: Any) {
        if (isFormValid()) {
            ...
            Model.instance.saveReviewImage(image: self.selectedImageVar!, reviewId: reviewId) { imageUrl in
                if imageUrl != "" {
                    let review = Review.createReview(id: reviewId, movieName: self.movieName.text!, releaseYear: Int64(self.yearText.text!)!, genre: self.genreText.text!, imageUrl: imageUrl, rating: Int64(self.ratingStars.rating), review: self.reviewText.text!, userId: Auth.auth().currentUser!.uid, lastUpdated: 0)
    ...
    func isFormValid() -> Bool {
        ...
        checks: if selectedImageVar == nil {
            isValid = false
            displayAlert(message: "Please choose an image for the movie")
            break checks
        }
        else if ((self.movieName.text?.isEmpty ?? true) || (yearText.text?.isEmpty ?? true) || (genreText.text?.isEmpty ?? true) || (reviewText.text?.isEmpty ?? true)){
            isValid = false
